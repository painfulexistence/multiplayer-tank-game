...
export class MyRoom extends Room<MyRoomState> {
  ...
  onCreate (options: any) {
    ...
    this.onMessage("move", (client, message: { direction: Direction }) => {
      ...
      let canMove = true;
      const playerRect = this.getRect({ x: newX, y: newY, direction: currPlayer.direction }, TANK_WIDTH, TANK_WIDTH); // FIXME: handle tank rotation
      this.state.walls.forEach(wall => {
        const wallRect = this.getRect({ x: wall.x, y: wall.y, direction: Direction.Up }, WALL_WIDTH, WALL_HEIGHT);
      ...
      currPlayer.x = Math.max(TANK_WIDTH / 2, Math.min(MAP_WIDTH - TANK_WIDTH / 2, currPlayer.x)); // FIXME: handle tank rotation
      currPlayer.y = Math.max(TANK_HEIGHT / 2, Math.min(MAP_HEIGHT - TANK_WIDTH / 2, currPlayer.y)); // FIXME: handle tank rotation
    });
  ...
  private getRect(entity: { x: number; y: number, direction: Direction }, width: number, height: number): Rect {
    if (entity.direction == Direction.Left || entity.direction == Direction.Right) {
      return {
        x: entity.x - width / 2,
        y: entity.y - height / 2,
        w: width,
  ...
  private update(dt: number) {
    ...
    this.state.enemies.forEach((enemy, id) => {
      ...
      enemy.x = Math.max(TANK_WIDTH / 2, Math.min(MAP_WIDTH - TANK_WIDTH / 2, enemy.x)); // FIXME: handle tank rotation
      enemy.y = Math.max(TANK_HEIGHT / 2, Math.min(MAP_HEIGHT - TANK_HEIGHT / 2, enemy.y)); // FIXME: handle tank rotation
    ...
    this.state.bullets.forEach((bullet, bulletId) => {
      ...
      const bulletRect = this.getRect(bullet, BULLET_WIDTH, BULLET_HEIGHT);
      ...
      this.state.players.forEach((player, playerId) => {
        const playerRect = this.getRect(player, TANK_WIDTH, TANK_HEIGHT);
        if (bullet.ownerId !== playerId && this.checkCollision(bulletRect, playerRect)) {
      ...
      this.state.enemies.forEach((enemy, enemyId) => {
        const enemyRect = this.getRect(enemy, TANK_WIDTH, TANK_HEIGHT);
        if (!bullet.ownerId.startsWith("enemy") && this.checkCollision(bulletRect, enemyRect)) {
          this.state.enemies.delete(enemyId);
          this.state.bullets.delete(bulletId);
        }
      });

      if (bullet.x < 0 - BULLET_WIDTH / 2 || bullet.x > MAP_WIDTH + BULLET_WIDTH / 2 || bullet.y < 0 - BULLET_WIDTH / 2 || bullet.y > MAP_HEIGHT + BULLET_WIDTH / 2) { // FIXME: handle bullet rotation
        this.state.bullets.delete(bulletId);
      }
    });
  ...
  onJoin (client: Client) {
    ...
    const position = PLAYER_SPAWN_POINTS[playerCount] || PLAYER_SPAWN_POINTS[0];
